...
```bash
uv run -m stkoe panel add ds1 index m1                  # 注册 panel（keys 由 index 推断 = sym+date；m1 为成员，缺省 asof join）
uv run -m stkoe panel add ds2 index m1:asof m2:left     # 成员可配 join：m1 asof、m2 left（等值）
