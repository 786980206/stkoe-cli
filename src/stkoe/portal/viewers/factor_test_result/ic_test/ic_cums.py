"""IC累计页面 - 原始IC、分组调整IC、RankIC、分组调整RankIC累计序列"""

import panel as pn
from panel_splitjs import Split
from stkoe.portal.models.factor_test_result import FactorTestResultModel

desc = """## IC累计

将IC序列逐日累加，形成累计IC曲线。累计IC曲线持续上升说明因子具有稳定的预测能力。

$$CumIC(t) = \\sum_{s=1}^{t} IC(s)$$

### 筛选条件

支持按日期范围筛选，如 `'2020-01-01<=date<=2020-12-31'`
"""

IC_CUMS_TYPES = [
    ("原始IC累计", "ic"),
    ("分组调整IC累计", "gic"),
    ("Rank IC累计", "rank_ic"),
    ("分组调整Rank IC累计", "rank_gic"),
]


class ICCumsView:
    """IC累计视图 - 控制工具栏 + 图表展示 + 数据表格"""

    def __init__(self, model: FactorTestResultModel) -> None:
        self.model = model
        self.desc = desc

        self.ic_type_input = pn.widgets.Select(
            value="原始IC累计",
            options=[label for label, _ in IC_CUMS_TYPES],
            width=150,
        )
        self.condition_input = pn.widgets.TextInput(
            placeholder="输入筛选条件, 如: '2020-01-01<=date<=2020-12-31'"
        )
        self.view_button = pn.widgets.Button(name="查看", color="primary")
        self.linked_axes_switch = pn.widgets.Switch(name="轴联动")
        self.data_button = pn.widgets.Button(name="数据", color="primary")

        self.graph_result = pn.Accordion(
            desc, scroll=True, sizing_mode="stretch_both", toggle=True, active=[0]
        )
        self.data_result = pn.Column(sizing_mode="stretch_both")

        self.toolbar = pn.Row(
            self.ic_type_input,
            self.condition_input,
            self.view_button,
            pn.HSpacer(),
            self.linked_axes_switch,
            self.data_button,
            scroll=True,
            sizing_mode="stretch_width",
            styles={"margin": "0 5px 0 0"},
        )
        self.split_content = Split(
            self.graph_result,
            self.data_result,
            collapsed=None,
            expanded_sizes=(50, 50),
            sizing_mode="stretch_both",
            styles={"border-top": "1px solid var(--panel-border-color)"},
            gutter_size=1,
        )

        self.layout = pn.Column(
            self.toolbar,
            self.split_content,
            styles={"height": "calc(100vh - 73px)"},
        )


class ICCumsController:
    """IC累计控制器"""

    def __init__(self, model: FactorTestResultModel, view: ICCumsView) -> None:
        self.model = model
        self.view = view

        view.view_button.on_click(self._onview_click)
        view.data_button.on_click(
            lambda _: setattr(view.split_content, 'collapsed', None if view.split_content.collapsed == 1 else 1)
        )

    def _onview_click(self, event) -> None:
        view = self.view
        ic_type_label = view.ic_type_input.value
        condition = view.condition_input.value or None
        linked_axes = view.linked_axes_switch.value

        loading = pn.indicators.LoadingSpinner(value=True, width=25, height=25)
        view.graph_result[:] = [loading]
        view.data_result[:] = [loading]

        ic_key = next(k for label, k in IC_CUMS_TYPES if label == ic_type_label)
        ic = self.model.ic_test

        if condition:
            plot_fn = getattr(ic, f"plot_{ic_key}_date_cums_by_condition")
            calc_fn = getattr(ic, f"calc_{ic_key}_date_cums_by_condition")
            plot = plot_fn(condition)
            data = calc_fn(condition)
        else:
            plot_fn = getattr(ic, f"plot_{ic_key}_date_cums_all")
            calc_fn = getattr(ic, f"calc_{ic_key}_date_cums_all")
            plot = plot_fn()
            data = calc_fn()

        view.graph_result[:] = [
            (
                plot.opts.get("plot").kwargs.get("title"),
                pn.panel(plot, sizing_mode="stretch_both", linked_axes=linked_axes),
            )
        ]
        view.data_result[:] = [
            pn.pane.Markdown("**聚合统计**"),
            pn.pane.Perspective(
                data.to_pandas(),
                columns=list(data.columns),
                sizing_mode="stretch_both",
                settings=False,
            )
        ]


class ICCumsViewer:
    """IC累计页面入口"""

    def __init__(self, model: FactorTestResultModel) -> None:
        if model.ic_test is None:
            self.view = None
            self.controller = None
        else:
            self.view = ICCumsView(model)
            self.controller = ICCumsController(model, self.view)

    @property
    def layout(self):
        if self.view is None:
            return pn.pane.Markdown(
                "## 无可用 IC 测试结果\n\n请将 ICTestResults.pkl 放入 `results/` 目录"
            )
        return self.view.layout
