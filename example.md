...
# 9.3 观察新桶出现、旧桶仍在（yearly：panel/fieldset 出现 part=2026 新桶，2024/2025 不动）
find example-data/panel/ds1 -name data.parquet
# → example-data/panel/ds1/part=2024/data.parquet
# → example-data/panel/ds1/part=2025/data.parquet
# → example-data/panel/ds1/part=2026/data.parquet
# （factor/tester 的样本键集合来自筛选参照 idx2——仅 2024 年前 100 个交易日，
#   2026 新数据不在样本池内，故它们仍只有 part=2024）
...
把上面全部步骤（§0–§11 的非交互部分）汇总为一个可直接运行的 bash 脚本——从头跑一遍即完成
「mock 造数 → 源头登记 → panel/fieldset 物化 → sample/feature/factor/tester → 增量更新 →
stat 测试器 → 血缘可视化」全流程。脚本开头自动清理旧产物（`example-data/` 与
`stkoe.example.json`），可重复运行；`serve` / `gclient.py` 交互部分（§11 上半）需另开终端手动执行。
PowerShell 用户：把 `export STKOE_CONFIG=...` 换成 `$env:STKOE_CONFIG='...'`，`find` 输出检查
可跳过或改用 `Get-ChildItem -Recurse`。
