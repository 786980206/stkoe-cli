...
def test_select_partition(root):
    _build(root)
    lf = data.select("t1", partition="year=2020")
    got = lf.collect()
    assert sorted(got["year"].unique().to_list()) == [2020]
    assert got.height == 2
    # 前缀匹配
    assert data.select("t1", partition="year").collect().height == 5
...
def test_select_where_date(root):
    ...
    df = _build(root)
    got = data.select("t1", where="date>=2021-01-01").collect()
    assert got.height == df.filter(pl.col("date") >= pl.date(2021, 1, 1)).height
    assert got["date"].min() >= datetime.date(2021, 1, 1)


def test_select_where_range(root):
    _build(root)
    got = data.select("t1", where="2020-01-01<=date<=2021-12-31").collect()
    assert got.height == 4


def test_select_where_numeric(root):
    _build(root)
    got = data.select("t1", where="r>=0.05").collect()
    assert got.height == 2
...
def test_select_columns(root):
    _build(root)
    got = data.select("t1", columns=["sym", "r"]).collect()
...
def test_select_pruning_equals_full(root):
    ...
    _build(root)
    lf = data.select("t1", partition=["year=2020", "year=2022"], where="r>=0.0")
    got = lf.collect()
    full = data.select("t1").collect()
    expect = full.filter(
...
def test_select_where_expr(root):
    _build(root)
    got = data.select("t1", where=pl.col("r") < 0).collect()
    assert got.height == 2
...
def test_select_partition_no_match(root):
    _build(root)
    lf = data.select("t1", partition="year=1999")
    assert lf.collect().height == 0
...
def test_schema(root):
    _build(root)
    s = data.schema("t1")
    assert "year" in s  # hive 分区列
    assert s["date"].is_temporal()
